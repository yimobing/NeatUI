...
;(function root(root, factory) {
    ...
    var net = {};
    // 两种调用方式：
    // 1.DOM方式调用： neuiInputsearch(elem, options);
    // 2.JQ方式调用： neuiInputsearch(options, $(elem));
    var neuiInputsearch = function(elem, options){
        var caller = tools.isJsonObject(options) ? "DOCUMENT" : "JQUERY"; // 调用方式
        var opts = tools.isJsonObject(options) ? options : elem;
        var ele = tools.isJsonObject(elem) ? 
        ...
        return new inputDrop(ele, opts, caller);
    };
    ...
    //================================================================
    function inputDrop(elem, options, method){
        var me = this;
        var defaults = {
            // 控件调用的两种方式：
        ...
        // 全局赋值
        this.$method = method; // 调用方式。 值： "DOCUMENT" DOM方式,  "JQUERY" JQ方式
        this.$elem = elem;
        ...
        this.$idElements = document.getElementById(selector);
        this.$classElements = document.getElementsByClassName(selector);
        ...
        if(this.$idElements != null){
            this.$dom = this.$idElements;
        }else{
            Array.prototype.forEach.call(this.$classElements, function(element){
                var newClassName = element.className + ' ' + tools.generateRandomChar();
                element.className = newClassName;
                me.$dom = element;
               //console.log('domXX：', me.$dom);
            })
        }
        //end testing
        ...
        //         var newClassName = element.className + ' ' + tools.generateRandomChar();
        //         element.className = newClassName;
        //         var newElement = document.getElementsByClassName(newClassName)[0];
        //         me.$dom = newElement;
        //         console.log('domxx：', element)
        //         me.init();
        //     })
        // }
    };
    ...
    inputDrop.prototype.init = function(){
        var me = this;
        // testing
        if(me.$method == 'DOCUMENT'){
            me.$dom.addEventListener(me.$opts.event, function(){
                me.createControl();
            })
        } else if(me.$method == 'JQUERY'){
            me.createControl();
        }

        // if(me.$method == 'DOCUMENT'){
        //     if(me.$idElements == null){ // 使用多个相同class节点调用控件时
        //         Array.prototype.forEach.call(me.$classElements, function(element){
        //             // 给每个节点添加不同的class以示区分
        //             var newClassName = element.className + ' ' + tools.generateRandomChar();
        //             element.className = newClassName;
        //             var newElement = document.getElementsByClassName(newClassName)[0];
        //             me.$dom = newElement; // 外面先赋值一次
        //             me.$dom.addEventListener(me.$opts.event, function(){
        //                 me.$dom = newElement; // 里面要再赋值一次,否则不起作用
        //                 console.log('DomYY：', me.$dom)
        //                 me.createControl();
        //             })
        //         })
        //     }else{ // 使用id节点调用控件
    ...
    inputDrop.prototype.createControl = function(){
        var me = this;
        var opts = me.$opts;
        ...
        // 判断数据源
        var inputValue = tools.getElementValue(me.$dom); // 当前输入框元素的值
        var dataSource = me.$method == 'DOCUMENT' ? opts.inputBack({value: inputValue}) : source;
        ...
        if(me.$opts.event == 'click' && me.$opts.value.toString().replace(/([ ]+)/g, '') !== '') {
            tools.giveValueToElement(me.$dom, me.$opts.value);
        }
        ...
        docDiv.className = 'neInputsearch ne_input_search_' + tools.generateRandomChar() + _eClassNameStr;
        opts.id === 'default' ||  opts.id.replace(/([ ]+)/g, '') === '' ? '' : docDiv.id = opts.id;
        width == 'auto' ? '' : docDiv.style.width = width + 'px';
        docDiv.style.zIndex = zIndex;
        docDiv.innerHTML = allHtml;
        if(me.$root) me.$root.remove();
        me.$dom.after(docDiv);
        console.log('当前拼接的DOM-ZZ：', me.$dom); // testing
        // 全局赋值
        me.$root = document.getElementsByClassName('neInputsearch')[0]; // 根节点
        ...
        if(typeof me.$oldId == 'undefined'){ // 第1次时才赋值
            me.$oldId = me.$dom.getAttribute('data-bh') == null ? '' : me.$dom.getAttribute('data-bh'); // 旧的隐藏值
        }
        if(typeof me.$oldValue == 'undefined'){ // 第1次时才赋值
            me.$oldValue = tools.getElementValue(me.$dom); // 旧的显示值
        }
        // 触发事件 
        if(me.$ul != null) me.selected(); // 选中下拉项事件
     ...
     * 选择某一项
     */
    inputDrop.prototype.selected = function(){
        var me = this;
     ...
     * 关闭按钮
     */
    inputDrop.prototype.close = function(){
        var me = this;
     ...
     */
    function fnFillValue2Box(ps_this, ps_bh, ps_val){
        var me = ps_this;
        if(me.$opts.autoFill.selectedFill || (child.length == 1 && fnGetIsOneFill(me))){
            me.$dom.setAttribute('data-bh', ps_bh); // 给当前元素赋隐藏值
            tools.giveValueToElement(me.$dom, ps_val); // 给当前元素赋显示值
        }
