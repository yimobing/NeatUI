 ...
 (function ($) {
    ...
    *                                       通用方法库
    ...
    var methods = {
        ...
        init:function(options){
            ...
            var defaults = {
                ...
                window:{ // 窗口布局(可选)
                    ...
                    pickBtnText: '确定', // 拾取器按钮文本(可选),默认'确定'
                    polygon: { // 多边形覆盖物(可选)
                        enable: false, // 是否启用多边形覆盖物(可选),默认false
                        ...
                        poiRadius: 1000,  // 指定范围M米(半径为M米内的POI)(可选),默认1000米
                        poiNum: 5, // 多边形点数N个(列举出N个POI)(可选),默认10个
                        // 按钮文本
                        labelText: '拖动多边形上的小正方形，可更改范围', // 提示文本(可选)
                        btnNewText: '创建多边形', // 新建按钮文本(可选)
                        btnDelText: '删除多边形', // 删除按钮文本(可选)
                        btnSaveText: '保存多边形点坐标', // 保存按钮文本(可选)
            ...
            var dPickup = dWindow.pickup,
                dPickBtnText = dWindow.pickBtnText == '' ? '确定' : dWindow.pickBtnText,
                dPosition = dWindow.position,
                dPoly = dWindow.polygon,
                dClosed = dWindow.closed;
            // test2 下2行
            var dDrawPoly = dPoly.enable;
            // if(dDrawPoly) dPickup = true; // 启用多边形时强制开启坐标拾取系统
            
             // 循环
             return this.each(function(u){
                var $this = $(this); // 当前控件元素对象
                ...
                function createBdmap(){
                    ...
                    */
                    ...
                    if(dPickup){
                        ...
                        var pickupHTML = [
                            '<div class="bdmap_point' + _sonEleClass + '">',
                                ...
                                '<button type="button" data-type="copy" id="bdmap_point_copy">复制</button>',
                                '<button type="button" data-type="save" id="bdmap_point_confirm">' + dPickBtnText + '</button>',
                            '</div>'
                        ].join('\r\n');
                    ...
                    if(dDrawPoly){ //   test2
                        var plyHTML = [
                            '<div class="bdmap_draw' + _sonEleClass + '">',
                                '<span>' + dPoly.labelText + '</span>', 
                                '<button type="button" data-type="del" id="bdmap_draw_del">' + dPoly.btnDelText + '</button>',
                                '<button type="button" data-type="new" id="bdmap_draw_new">' + dPoly.btnNewText + '</button>',
                                '<button type="button" data-type="save" id="bdmap_draw_save">' + dPoly.btnSaveText + '</button>',
                            '</div>'
                        ].join('\r\n');
                        if($rootNode.find('.bdmap_draw').length == 0){
                        ...
                        $('#bdmap_draw_new').on('click', function(){
                            ...
                            // 再强制重建多边形   
                            methods.createPolygon({forceCenterPoly: true});
                            // 最后获取多边形的点坐标，并回调
                            var tmpPointArr = [];
                            ...
                            // console.log('tempArr:', tmpPointArr)
                            var coordArr = utilities.converCoordinateArray2StringArray(tmpPointArr);
                        ...
                        $('#bdmap_draw_del').on('click', function(){
                            ...
                            // console.log('tempArr:', tmpPointArr)
                            var coordArr = utilities.converCoordinateArray2StringArray(tmpPointArr);
                            if(dPoly.btnDelCallBack) dPoly.btnDelCallBack(coordArr);
                            // 最后清除多边形覆盖物
                            methods.removePolyOverlay();
                        })
                         ...
                         $('#bdmap_draw_save').on('click', function(){  
                            ...
                            // console.log('tempArr:', tmpPointArr)
                            var coordArr = utilities.converCoordinateArray2StringArray(tmpPointArr);
                            if(dPoly.btnSaveCallBack) dPoly.btnSaveCallBack(coordArr);
                        })
                    } 
                    ...
                    // test2 下2行
                    $.CENTERDEFAULTPOINT = defaults.point; // 默认中心点坐标
                    $.CENTERPOINT = dPoint; // 中心点坐标
                    $.USENUMC = dIsNumc; // 中心点是根据城市还是坐标来设置. true 根据坐标(默认), false 根据城市
                    $.WINDOW = dWindow;
                    $.POLYPOINTS = dPoly.points;
                    ...
                    //=====设置默认中心点标注点
                    if(dPickup || dDrawPoly){ // 当开启坐标拾取系统 test2 本行
                        var boxValue = tagname == 'input' || tagname == 'textarea' ? $this.val() : $this.text();
                    ...
                    if(settings.mouseClick){
                        map.addEventListener('click', function(e){
                            ...
                            if(dPickup){ // 当开启坐标拾取系统
                                addCenterMarker(longitude + ',' + latitude, dLabel, false); // 重置中心点标注 test2
                                // 要注释掉这句话,不能拖动地图时,多边形就消失了
                            }
                        })
                    }
                    // 鼠标双击
                    ...
                    if(settings.mouseDrag){
                        ...
                        ['dragstart','dragging','dragend'].forEach(function(item, index){
                            ...
                            map.addEventListener(item, function(e){
                                ...
                                if(item == 'dragend'){
                                    if(dPickup){ // 当开启坐标拾取系统
                                        addCenterMarker(lng + ',' + lat, dLabel, false); // 重置中心点标注 test2
                                        // 要注释掉这句话,不能拖动地图时,多边形就消失了
                                    }
                                }
                            })
        ...
        /**
         * 创建多边形覆盖物 test2
         * @param {object} ps_opts 参数对象(可选). 格式 {forceCenterPoly: "是否根据点坐标强制重绘多边形,值true或false"}
         */
        createPolygon: function(ps_opts){
            var isForcedPly = typeof ps_opts == 'undefined' ? false : (ps_opts.forceCenterPoly == true ? true : false); // 默认false
            var map = $.MAPER;
            var dPoint = $.CENTERPOINT, // 中心点坐标
                dPoly = $.WINDOW.polygon, 
                dDrawPoly = dPoly.enable; // 是否启用多边形
            var dPointsArray = $.POLYPOINTS != null ? $.POLYPOINTS : dPoly.points; // 多边形点数组
            if($.USENUMC && dPoint == $.CENTERDEFAULTPOINT) return;
            ...
            if(dDrawPoly){
                ...
                var dPoiPoint = dPoly.poiPoint == null ? dPoint : dPoly.poiPoint;
                    dPoiRadius = dPoly.poiRadius,
                    dPoiNum = dPoly.poiNum;
                if($('#bdmap_point').length != 0) dPoiPoint = $('#bdmap_point_text').val();
                // console.log('多边形是否已有点坐标：', isPolyArray, '\n多边形点数组：', dPointsArray, '\n多边形中心点坐标：',dPoiPoint);
                // console.log('-------------------------');
                // console.log('dPoiPoint:', dPoiPoint);

                var mlyArr = dPoiPoint.split(','),
                    mlyLng = mlyArr[0],
                    mlyLat = mlyArr[1];
                var mPoint = new BMap.Point(mlyLng, mlyLat);
                //
                var polyPointArray = []; // 多边形点数组
                var polyOptions = { // 多边形配置项
                    strokeColor: 'blue',
                    strokeWeight: 2,
                    strokeOpacity: 0.5
                }
                ...
                // 获取中心点坐标指定范围内的N个点
                function displayPOI(){
                    if(isPolyArray){ // 根据已有的N个点坐标，创建多边形
                        for(var i = 0; i < dPointsArray.length; i++){
                            var row = dPointsArray[i];
                            var tmpArr = row.split(',');
                            var lng = tmpArr[0], lat = tmpArr[1];
                        ...
                        fnEditBack(); // 回调
                        //
                    }else{ // 根据中心点坐标获取指定范围内的M个点，创建多边形
                        var myGeo = new BMap.Geocoder(); // 创建地址解析实例
                        var locateOptions = {
                            poiRadius: dPoiRadius,  // 半径为M米内的POI，默认100米
                            numPois: dPoiNum // 列举出N个POI，默认10个
                        }
                ...
                function fnEditBack(){
                    // console.log('xxx1-polyPointArray:', polyPointArray)
                    var ply = new BMap.Polygon(polyPointArray, polyOptions);
                    if(dPoly.editable) ply.enableEditing(); // 设置可编辑
                   ...
                   // map.clearOverlays();
                    ...
                    ply.addEventListener("lineupdate", function(e){ // 拖动边线进行编辑时监听事件
                        ...
                        var coordArr = utilities.converCoordinateArray2StringArray(path);
                        // console.log('编辑时多边形点数组2：', coordArr)
                        $.POLYPOINTS = coordArr; // 全局赋值
                        if(dPoly.editCallBack) dPoly.editCallBack(coordArr); // 回调
                    })

                    var coordArr = []
                    for(var i = 0; i < polyPointArray.length; i++){
                        coordArr.push(polyPointArray[i].lng + "," + polyPointArray[i].lat);
                    }
                    $.POLYPOINTS = coordArr; // 全局赋值
    ...
    */
    ...
    function addCenterMarker(ps_point, ps_text, ps_viewport){
        ...
        // 全局赋值
        $.ENABLEVIEWPORT = isViewport;
        $.CENTERPOINT = ps_point;
        //
    ...
    *                                       全局对象
    ...
    $.extend({
        ...
        NODE: { // 节点名称
            ROOTCLASS: '.neBdmap', // 地图根节点class名称
            // ROOTID: '#' + 'neBdmap_' + utilities.generateRandomChar(), // 地图根节点id名称
            ROOTID: '#' + 'neBdmap_', // 地图根节点id名称
    ...
    *                                       对外暴露控件接口
    ------------------------------------------------------------------------------------------------*/
    $.fn.neuiBaiduMap = function(method){
        if(methods[method]){
...
//  // test1
//  var BDMAP = function(method){
//      var $dom = $('body');
//      $dom.neuiBaiduMap.apply(null, Array.prototype.slice.call(arguments, 0));
//  }