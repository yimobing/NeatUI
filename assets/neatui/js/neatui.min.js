	...
	*/
	...
	openIframe:function(opt){
		...
		var defaults = {
			url: '', //链接地址
			caption: '', //窗口标题(可选)
			top: '2.5%', //窗口距离顶部的距离(可选)（数字或百分比，eg.25%)
			width: '98%', //窗口宽度(可选)（数字或百分比，eg1.1200, eg2.98%）
			titleHeight:45, //标题栏高度(可选)
			...
			params: { //自定义其它参数(可选)
				frameId: 'ne-frame-id-' + Math.random().toString(36).substr(2), //ifame的ID属性(可选)
				frameName: 'ne-frame-name', //iframe的Name属性(可选)
				frameZindex: 51, //iframe层级z-index(可选)
				frameRadius: true,  //iframe四个角是否圆角(可选),默认true
				frameBorder: true, //iframe是否有边框(可选),默认true
				maskId: 'ne-frame-mask', //遮罩id(可选)
				maskZindex: 50, //遮罩层级z-index(可选)
				animation:{ //iframe打开时是否启用动画效果(可选)
					enable: false, //默认false不开启(可选)
					direction: 'left' //动画方向(可选). left 向左(默认), top 向上
				}
			}, 
		...
		if(ps_url != '' && typeof(ps_url) != 'undefined'){
			...
			$('body').append(_html).animate({}, function(){
				...
				// ios bug： 解决部分ios设备中iframe的宽、高被子页面内容撑开，导致子页面中fixed类型的元素无法固定在底部或左右两侧的问题
	...
	openAside: function(opt){
		...
		var defaults = {
			...
			adapWidth: 'auto', //窗口自适应大小时的最大宽度(可选). 默认auto. (此参数一般用于pc端自定义窗口大小)
