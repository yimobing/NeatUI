...
var neui = {
	...
	*/
	...
	openIframe:function(opt){
		...
		var defaults = {
			...
			openCallBack: function(){}, // 打开时的回调函数(可选)
		...
		var ps_url = settings.url,
			...
			ps_number = ps_top.toString().indexOf('%') >= 0 ? 0 
			: 
			( 	
				ps_top.toString().indexOf('px') >= 0 ? parseFloat(ps_top.toString().replace(/px/g, ''))
				:
				( 2 * isNaN(parseFloat(ps_top)) ? 0 : parseFloat(ps_top)) 
			),
			ps_h_has_percent = 100 - 2 * ps_top.toString().replace(/[\%]/g,'') + '%',
			ps_h_has_calc = 'calc(100% - ' + 2 * ps_number + 'px)',
			ps_height = ps_number == 0 ? ps_h_has_percent : ps_h_has_calc,
		...
		if(ps_url != '' && typeof(ps_url) != 'undefined'){
			...
			// 检测到混合内容风险时。即当前页面https协议，嵌入页面为http协议时浏览器会阻止打开，此时要提示用户一下 add 20260803-1
			if(this.checkMixedContentRisk(ps_url)) {
				var tip = '打开链接失败！<br>原因：目标链接地址为HTTP协议，当前页面环境为HTTPS协议，无法直接嵌入。请将目标链接地址升级为HTTPS协议。';
				this.alertDialog(tip);
				return;
			}
			...
			var outerH = ps_number == 0 ? (winH * (ps_h_has_percent.replace(/[\%]/g, '') / 100)).toFixed(0) : winH - 2 * ps_number,
				...
				// edit 20250624-1
				// top = ps_top,
				top = ps_top.toString().indexOf('%') >= 0 ? ps_top : ps_top + 'px';
				
				width = ps_width.toString().indexOf('%') >= 0 ? ps_width : ps_width + 'px',
	...
	/**
	 * 综合判断：在当前页面环境下，加载某个url链接是否会触发https和http两种协议混合内容报错
	 * add 20260803-1
	 * @param {string} iframeUrl iframe地址
	 * @returns {boolean} true=会被浏览器拦截，false=安全
	 */
	 checkMixedContentRisk: function(iframeUrl) {
		var pageHttps = this.isCurrentPageHttps();
		var urlHttpsFlag = this.isUrlHttps(iframeUrl);
		// 场景1：父页面不是https，不存在混合内容问题
		if (!pageHttps) {
			return false;
		}
		// 场景2：url明确https，安全
		if (urlHttpsFlag === true) {
			return false;
		}
		// 场景3：相对路径 /index.html 或协议相对地址 //xxx.com，跟随页面https，安全
		if (urlHttpsFlag === null) {
			return false;
		}
		// 场景4：父页面https，目标明确http → 触发混合内容拦截！
		return true;
	},
	 ...
	 * true = 确定https
	 * false = 确定http
	 * null = 相对路径 index.html，或协议相对地址 //xxx.com（需要基于当前页面协议动态判断）
	 */
	isUrlHttps: function(url) {
		if (!url || typeof url !== 'string') {
			return false;
		}
		// 正则捕获协议部分：https: / http: 或者 // 协议相对路径
		var regProtocol = /^(https?:)\/\/|^\/\//i;
		var match = url.match(regProtocol);
		if (!match) { // 没有协议，如 /index.html 相对路径，不属于http/https，返回null做特殊标记
			return null;
		}
		var proto = match[1];
		if (proto) { // 有明确协议 http: / https:
			return proto.toLowerCase() === 'https:';
		} else { // 匹配到 // 协议相对地址，返回null做特殊标记
			return null;
		}
	},
	...
	openAside: function(opt){
		...
		var defaults = {
			...
			maskOpacity: 0.6, //  遮罩透明度(可选),  默认0.6. 值：0到1. 0 表示完全透明, 1 完全不透明.
			animate: { //动画(可选)
				enable: true, //是否启用
		...
		var _mMaskOpacityStr = maskOpacity == 0.6 ? '' : 'opacity:' + maskOpacity + ';';
		// 样式
		var _offLeftStr = offLeft == '' || offLeft == 0 ? '' :
			(
				offLeft.toString().indexOf('%') >= 0 ?
					' left: ' + offLeft + ';'
					:
					' left: ' + offLeft + 'px;'
			),
			_offRightStr = offRight == '' || offRight == 0 ? '' : (
				offRight.toString().indexOf('%') >= 0 ?
					' right: ' + offRight + ';'
					:
					' right: ' + offRight + 'px;'
			),
			_offTopStr = offTop == '' || offTop == 0 ? '' : (
				offTop.toString().indexOf('%') >= 0 ?
					' top: ' + offTop + ';'
					:
					' top: ' + offTop + 'px;'
			),
			_offBottomStr = offBottom == '' || offBottom == 0 ? '' : (
				offBottom.toString().indexOf('%') >= 0 ?
					' bottom: ' + offBottom + ';'
					:
					' bottom: ' + offBottom + 'px;'
			);
		...
		var _maskStyle = ' style="' + _mZindexStr + _mMaskOpacityStr + '"';
		...
		$(allHtml).appendTo('body').animate({}, function(e){
			...
			$(eleRoot).find('.as-lay-footer button').on('click', function(){ //点击按钮
				...
				if(settings.btnClickClose) shutSide($(this)); // 关闭窗口这一句要放最后,不然如果要读取窗口中的内容将无法读到
			})
			// 遮罩回调
			$(eleRoot).find('.aside-mask').on('click', function(){
				if(settings.noneSelfClickClose) shutSide($(this));
			})
			// 关闭按钮、返回按钮回调 edit 20241217-1
			$(eleRoot).find('.header-top-back, .header-top-cross').on('click', function(){
				...
				shutSide($(this)); // 关闭窗口这一句要放最后,不然如果要读取窗口中的内容将无法读到
			})
	...
	doneAsyncEvent: function(ps_opts){
		...
		if(settings.getData){
			...
			if(result instanceof Promise){ // Promise对象(异步取数)
				...
				})["catch"](function(err){ // .catch 写成 ["catch"] 是为了兼容ie8
					alert(err);
				})
			}
	...
	*/
	...
	exportWeb2Image: function(options){
		...
		_this.showAnimate('努力导出图片中...');
		// 判断是否引入html2canvas
		if(typeof html2canvas === 'undefined'){
			_this.destroyAnimate();
	...
	goToTop: function(options){
		var defaults = {
			...
			zIndex: 2, // 层次，默认2(可选)
			direction: "right", // 方向，默认right(可选)。值： right 右侧, left 左侧
			bottom: "5%", // 距离底部的距离，默认5%(可选)。可使用的值类型有：5或5px或5%
			opacity: .6, // 透明度，默认0.6(可选)。值：0到1，1表示完全不透明，0 表示完全透明
			title: "TOP", // 文字，默认TOP(可选)
			showTitle: true, // 是否显示文字，默认true(可选)。值为false时只显示一个箭头
			distance: 200, // 往下滚动多少距离时才显示本控件，默认200px(可选)。可使用的值类型有：5或5px
			alwaysShow: false // 是否一直显示本控件，默认false(可选)。值为true时，distance 参数失效。
		}
		var settings = $.extend(true, {}, defaults, options || {});
		if(settings.enable == false) return;
		var z_index = settings.zIndex,
			direction = settings.direction,
			bottom = /^\d+$/.test(settings.bottom) ? settings.bottom + 'px' : settings.bottom,
			opacity = isNaN(parseFloat(settings.opacity)) ? parseFloat(defaults.opacity) : parseFloat(settings.opacity),
			distance = isNaN(parseFloat(settings.distance.toString().replace(/px/g, ''))) ? defaults.distance : parseFloat(settings.distance.toString().replace(/px/g, ''));
		var _titleStr = settings.showTitle === false || settings.title.toString().replace(/\s+/g, '') === '' ? '<icon></icon>' : '<span>' + settings.title + '</span>';
		var _noTextClassName = settings.showTitle === false || settings.title.toString().replace(/\s+/g, '') === '' ? ' has-no-text' : '';
		var _style = '';
		if(z_index != defaults.zIndex) {
			_style += 'z-index: ' + z_index;
		}
		if(opacity != defaults.opacity) {
			_style += ';opacity: ' + opacity + '; filter: alpha(opacity = ' + (opacity * 100) + ');filter: progid:DXImageTransform.Microsoft.Alpha(opacity=' + (opacity * 100) + ');';
		}
		if(bottom != defaults.bottom) {
			_style += ';bottom: ' + bottom;
		}
		if(direction == 'right') {
			_style += ';right: 0; border-radius: 4px 0 0 4px;';
		}
		else if(direction == 'left') {
			_style += ';left: 0;  border-radius: 0 4px 4px 0;';
		}
		if(settings.alwaysShow == false) {
			_style += ';display: none';
		}
		_style = _style.indexOf(';') == 0 ? _style.toString().substring(1, _style.length) : _style;
		var _styleStr = ' style="' + _style + '"';
		var _html = [
			'<div class="ne-goToTop' + _noTextClassName + '"' + _styleStr + '>',
				_titleStr,
			'</div> <!--/.ne-goToTop-->'
		].join('\r\n');
		if($('.ne-goToTop').length == 0) {
			$('body').append(_html);
		}
