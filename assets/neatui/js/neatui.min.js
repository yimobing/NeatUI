	...
	*/
	...
	installmentPayment: function(options){
		...
		var payment = settings.payment != '等额本息' && settings.payment != '等额本金' ? '等额本息' : settings.payment,
			...
			amount = parseFloat(settings.total.toString().replace(/,/g, '')),
			rate = parseFloat(settings.rate.toString().replace(/\%/g, '')) / 1200, // 转成月利率
		...
		if(!regNumber.test(amount.toString())) amount = 0;
		if(!regNumber.test(rate.toString())) rate = 0;
		if(!regNumber.test(digit.toString())) digit = 2;
		// console.log('payment：', payment, '\nmonth：', month, '\namount：', amount, '\nrate：', rate, '\ndigit：', digit);
		// 计算并拼接HTML
		var html = '<div class="payments">';
		if(month == 0){
			html += '<div class="payments__nodata">对不起，当前贷款年限为0年(0期)，无法计算分期数据。</div>';
		}else if(rate == 0){
			html += '<div class="payments__nodata">对不起，当前贷款利率为0%，无法计算分期数据。</div>';
		}else{
			html += '<div class="payments__subtitle">（' + payment + '，贷款总额' + amount + '元，年利率' + settings.rate + '%，按揭' + month + '期）</div>';
			html += '<div class="payments__content">';
			// 等额本息
			if(payment == '等额本息'){
				var payEveryMonth = ( amount * rate * Math.pow(1 + rate, month) ) / ( Math.pow(1 + rate, month) - 1 ); // 每月还款金额
				if(isNaN(payEveryMonth)) payEveryMonth = 0;
				for(var i = 1; i <= month; i++){
					var principal = ( ( payEveryMonth - amount * rate) * Math.pow(1 + rate, i - 1) ), // 每月应还本金
						interest = ( ( amount * rate - payEveryMonth ) * Math.pow(1 + rate, i - 1) ) + payEveryMonth; // 每月应还利息
					// 转换
					principal = format.thousandth(parseFloat(principal).toFixed(digit)); // 四舍五入，保留N位小数
					interest = format.thousandth(parseFloat(interest).toFixed(digit)); // 四舍五入，保留N位小数
					html += [
						'<div class="stages flex align-center">',
							'<div class="stages__term">第' + i + '期</div>',
							'<div class="stages__money">',
								'<div class="stages__top">',
									'<div class="stages__monthly">&yen;' + format.thousandth(parseFloat(payEveryMonth).toFixed(digit)) + '</div>',
								'</div>',
			...
			if(payment == '等额本金'){
				for(var i = 1; i <= month; i++){
					var payEveryMonth = ( amount * ( 1 + (month - i + 1) * rate) ) / month, // 第N个月(n>=2)还款金额
						principal = amount / month, // 每月应还本金
						interest = ( (month - i + 1) * amount * rate ) / month;  // 每月应还利息
					// 转换
					payEveryMonth = format.thousandth((payEveryMonth).toFixed(digit)); // 四舍五入，保留N位小数
					principal = format.thousandth(parseFloat(principal).toFixed(digit)); // 四舍五入，保留N位小数
					interest = format.thousandth(parseFloat(interest).toFixed(digit)); // 四舍五入，保留N位小数
					html += [
						'<div class="stages flex align-center">',
