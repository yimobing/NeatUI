...
var neuiDistrict = {
    ...
    /**
     * 获取省份获取对应城市数据
     * @param {string} ps_province_name 省份名称
     * @returns {object} 返回城市数组对象
     */
    getCityData: function(ps_province_name){
        var provinceId = this.getProvinceIdByProvinceName(ps_province_name);
        var arr = [ ];
        for(var i = 0; i < iosCitys.length; i++){
            var row = iosCitys[i];
            if(row.parentId == provinceId){
                arr.push({ id: row.id, value: row.value });
            }
        }
        return {data: arr}
    ...
    /**
     * 获取城市对应区县数据
     * @param {string} ps_city_name 城市名称
     * @returns {object} 返回区县数组对象
     */
    getCountyData: function(ps_city_name){
        var cityId = this.getCityIdByCityName(ps_city_name);
        var arr = [ ];
        for(var i = 0; i < iosCountys.length; i++){
            var row = iosCountys[i];
            if(row.parentId == cityId){
                arr.push({ id: row.id, value: row.value });
            }
        }
        return {data: arr}
    ...
    getProvinceIdByProvinceName: function(ps_province_name){
        ...
        for(var i = 0; i < iosProvinces.length; i++){
            var row = iosProvinces[i];
            var id = row.id, value = row.value;
            if(ps_province_name.toString().replace(/([ ]+)/g, '') == value){
                province_id = id;
                break;
            }
        }
    ...
    getCityIdByCityName: function(ps_city_name){
        ...
        for(var i = 0; i < iosCitys.length; i++){
            var row = iosCitys[i];
            var id = row.id, value = row.value;
            if(ps_city_name.toString().replace(/([ ]+)/g, '') == value){
                city_id = id;
                break;
            }
        }
     ...
     /**
     * 通过区县名称获取区县编号
     * @param {string} ps_county_name 区县名称
    ...
    getCountyIdByCountyName: function(ps_county_name){
        ...
        for(var i = 0; i < iosCountys.length; i++){
            var row = iosCountys[i];
            var id = row.id, value = row.value;
            if(ps_county_name.toString().replace(/([ ]+)/g, '') == value){
                county_id = id;
                break;
            }
        }
...
var iosCountys = [
    ...
    {"id":"330113","value":"临平区","parentId":"330100"},
    {"id":"330114","value":"钱塘区","parentId":"330100"},
