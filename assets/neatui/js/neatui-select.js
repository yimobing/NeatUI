...
(function() {
	...
	//============解决IE9及以下版本不支持 html dom classlist而导致iosselect在ie中打开后下拉选项无法出现的问题 add 20171225-1 by chenhairui
	...
	//============/end 解决IE9及以下版本不支持 html dom classlist而导致iosselect在ie中打开后下拉选项无法出现的问题
		...
		var iosSelectUtil = {
			...
			attrToData: function (dom, index) { //IE11以下的浏览器不支持html dom .dataset,故需重写方法(注:20171225 by chenhairui)
				var obj = {};
				if(dom.dataset){ //非IE
					//console.log('dataset:',dom.dataset);
					//console.log('dom111:',dom.getAttribute('data-id'));
					for (var p in dom.dataset) {
						obj[p] = dom.dataset[p];
					}
					obj['dom'] = dom;
					obj['atindex'] = index;
				}else{ //IE浏览器 add by chenhairui 20171225-1
					for(var i=0;i<dom.attributes.length;i++){
						var key = dom.attributes[i].nodeName;
						if(/^data-\w+$/.test(key)){//判断是否以data-开头的属性
							var value = dom.attributes[i].nodeValue; //值
		...
		IosSelect.prototype = {
			...
			renderLevel: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, sixLevelId, levelData) {
				...
				//console.log('obj:',obj);
				//console.log('levelUlContainDom:',renderMap.levelUlContainDom.innerHTML);
				//console.log('kkk:',tmpHtml);
				//console.log('ooo:',renderMap.levelContainDom);
