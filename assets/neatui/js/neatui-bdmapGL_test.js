/**
 * [neuiBdmap]
 * 百度地图插件 纯JS原生版 (百度地图API GL版本，未完成，测试中)
 * 使用的百度地图API版本： JS API GL 版本
 * Version：v1.0.1
 * 还差的功能：参数整合
...
; (function (root, factory) {
    ...
    Widget.prototype = {
        ...
        createMap: function (elem) {
            ...
            var maper = new BMapGL.Map(container, platOptions);
            // 第2步 设置中心点坐标
        ...
        createMarker: function(options){
            ...
            var markPoint = new BMapGL.Point(longitude, latitude);;
            // 添加点标记
            var makeOptions = me.settings.markers.markOptions;
            var iconUrl = makeOptions.image.path + makeOptions.image.icon,
                iconSize = new BMapGL.Size(makeOptions.image.size, makeOptions.image.size),
                iconOptions = {
                    // offset: new BMapGL.Size(0, 0),
                    // imageOffset: new BMapGL.Size(0, 0),
                    anchor: new BMapGL.Size(5, 10) // 图标的定位锚点。此点用来决定图标与地理位置的关系，是相对于图标左上角的偏移值，默认等于图标宽度和高度的中间值
                };
            var markerOptions = { }
            if(makeOptions.image.enable){
                markerOptions.icon = new BMapGL.Icon(iconUrl, iconSize, iconOptions);
            }
            var oneMark = new BMapGL.Marker(markPoint, markerOptions);
            ...
            if (title != '') {
                ...
                var label = new BMapGL.Label(lbText, lbOptions);
        ...
        createPolygonOverlays: function (options) {
            ...
            if (typeof points != 'undefined') {
                for (var i = 0; i < points.length; i++) {
                    ...
                    for (var k = 0; k < one.length; k++) {
                        var row = one[k];
                        if (row instanceof BMapGL.Point == false) {
                            one[k] = new BMapGL.Point(row.lng, row.lat);
                        }
                    }
                    newPoints.push(one);
                }
            ...
            for (var i = 0; i < newPoints.length; i++) {
                ...
                var ply = new BMapGL.Polygon(onePoint, skinOptions);
                try {
                    if (editable) {
                        ply.enableEditing();
                    }
        ...
        getPointByCity: function (ps_city, callback) {
            ...
            var geoc = new BMapGL.Geocoder(); // 创建地理编码实例
            // 回调函数，用于处理地理编码请求后的结果
        ...
        setMapType: function (options) {
            ...
            var mtype1 = new BMapGL.MapTypeControl({
                mapTypes: dTypes,
                anchor: anchor,
                offset: new BMapGL.Size(dOffW, dOffH) // 偏移量
            });
            maper.addControl(mtype1);
            // 地图、卫星、三维（相当于地图、混合、三维) 有问题，不能用
           /*  var city = '泉州市'; // me.settings.city;
            var mtype2 = new BMapGL.MapTypeControl({
                anchor: anchor,
        ...
        setMapCopyright: function (options) {
            ...
            var cr = new BMapGL.CopyrightControl({
                anchor: anchor,
                offset: new BMapGL.Size(dOffW, dOffH)
            })
        ...
        */
        setRegionBoundary: function(name, callback){
            ...
            var maper = me.$opts.$maper;
            // 查询行政区域的边界
            var bdary = new BMapGL.Boundary(); // 创建行政区域搜索的对象实例
            bdary.get(name, function (rs) {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    for (var k = 0; k < tempArr.length; k++){
                        ...
                        geoPointArr.push(new BMapGL.Point(lng, lat));
                    }
        ...
        setLocationBoundary: function (name, callback) {
            ...
            var maper = me.$opts.$maper;
            // 使用本地检索方法获取某个地点的边界
            var local = new BMapGL.LocalSearch(maper, {
                renderOptions: {
                    map: maper
                }
            });
    ...
    var dotHelper = {
        ...
        _addInfoWindow: function (me, marker, point, options) {
            ...
            var infoDow = new BMapGL.InfoWindow(infoContent, infoOptions);  // 创建信息窗口对象 
        ...
        _mouseClick: function (me, maper, options) {
            ...
            maper.addEventListener('click', function (e) {
                ...
                if (me.settings.center.enableClickCreateNew) {
                    ...
                    var longitude = e.latlng.lng, latitude = e.latlng.lat;
    ...
    var polygonHelper = {
        ...
        */
        ...
        _addOverlayLabel: function (me, polyObj, options) {
            ...
            var finals = utils.combine(true, originals, options || {});
            var position = new BMapGL.Point(finals.longitude, finals.latitude),
            ...
            var btOpts = {
                position: position, // 指定文本标注所在的地理位置. eg.new BMapGL.Point(118.599547, 24.9246154) 
                offset: new BMapGL.Size(-50, -30) //设置文本偏移量  
                // offset: new BMapGL.Size(0, 0) //设置文本偏移量  
            }
            var label = new BMapGL.Label(txtHtml, btOpts);  // 创建文本标注对象  
            label.enableMassClear(); // 允许覆盖物被清除
        ...
        _addOverlayButton: function (me, polyObj, options) {
            ...
            var btOpts = {
                position: position, // 指定文本标注所在的地理位置. eg.new BMapGL.Point(118.599547, 24.9246154) 
                offset: new BMapGL.Size(-50, -30) //设置文本偏移量  
                // offset: new BMaBMapGL.Size(0, 0) //设置文本偏移量  
            }
            var label = new BMapGL.Label(btHtml, btOpts);  // 创建文本标注对象  
            label.enableMassClear(); // 允许覆盖物被清除
        ...
        _delPoint: function (me, i) {
            ...
            var newOverlay = [];
            for (var i in a) {
                newOverlay.push(new BMapGL.Point(a[i][0], a[i][1]));
            }
