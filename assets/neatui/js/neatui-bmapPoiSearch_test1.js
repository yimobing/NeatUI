 ...
 */
 ...
 (function(window) {
    ...
    NeBdPoiSearch.prototype.createUI = function() {
        ...
        // 关键词分组 add 20260622-1
        html += [
            '<div class="group-select-section">',
                '<div class="group-select-title"> 按分组选关键字 </div>',
                '<div class="group-select-content">',
                    '<ul>',
                        '<li> 住宅组 </li>',
                        '<li class="selected"> 店铺组 </li>',
                        '<li> 工业组 </li>',
                    '</ul>',
                '</div>',
            '</div>'
        ...
        // 已选分类计数（始终显示）
        html += '<div class="multi-select-section" style="display: none;">' +
                ...
                '<div class="quick-select-content">',
                    (function(){
                        ...
                        for (var i = 0; i < categoryKeys.length; i++) {
                            ...
                            _tmpHtml += '<button type="button" class="quick-btn" data-category="' + key + '">' + cat.icon + ' ' + cat.name + '</button>';
                        }
                        return _tmpHtml;
                    })(),
                    '<button type="button" class="quick-btn-more" id="moreBtn">更多 ↓</button>',
                '</div>',
            '</div>',
        ].join('\r\n')
        ...
        // 更多分类、更多关键词
        html += [
            '<div class="quick-select-section" id="moreSection" style="display: none;">',
    ...
    NeBdPoiSearch.prototype.updateSelectedCategoriesUI = function() {
        ...
        if (selectedCount) {
            if (count > 0) {
                selectedCount.parentNode.style.display = '';
                selectedCount.textContent = '已选 ' + count + ' 个';
            } else {
                selectedCount.parentNode.style.display = 'none';
            }
        }
        ...
        if (selectedCategories && selectedTags) {
            if (count > 0) {
                ...
                for (var i = 0; i < this.state.selectedCategories.length; i++) {
                    ...
                    html += 
                        [
                            '<span class="selected-tag">' + category.icon + ' ' + category.name + subcategoryText,
                                '<input type="text" value="1000">',
                                '<em> 米 </em>',
                                '<span class="tag-close" data-category="' + key + '">×</span>',
                            '</span>'
                        ].join('\r\n')
                }
