...
;(function($){
    ...
    MyBdp.prototype.createMap = function(){
        ...
        this.drawManager = new BMapLib.DrawingManager(map, {
            ...
            enableDrawingTool: me.settings.draws.enable, // 是否显示工具栏
            drawingToolOptions: { // 可选的输入参数，非必填项
                anchor: BMAP_ANCHOR_TOP_RIGHT, // 位置
                offset: new BMap.Size(5, 5), // 偏离值
                scale: 0.8, // 工具栏缩放比例
                drawingModes: me.settings.draws.drawingModes, // 工具栏上可以选择出现的绘制模式(可选),将需要显示的DrawingType以数组型形式传入，如[BMAP_DRAWING_MARKER, BMAP_DRAWING_CIRCLE] 将只显示画点和画圆的选项。其它值：// BMAP_DRAWING_MARKER, // 画点(画位置) // BMAP_DRAWING_CIRCLE, // 画圆 BMAP_DRAWING_POLYLINE, // 画线 BMAP_DRAWING_POLYGON, // 画多边形 BMAP_DRAWING_RECTANGLE // 画矩形  
            },
            enableCalculate: false, // 绘制是否进行测距(画线时候)、测面(画圆、多边形、矩形)
            circleOptions: me.settings.draws.drawingOptions, // 圆的样式
            polylineOptions: me.settings.draws.drawingOptions, // 线的样式
            polygonOptions: me.settings.draws.drawingOptions, // 多边形的样式
            rectangleOptions: me.settings.draws.drawingOptions // 矩形的样式
        })
        // 添加绘图切换事件
        this.drawCursorHand();
