 ...
 * [neuiMagnify]
 * 图片放大预览控件
 * [功能] 适用于PC端，让你可以像在windows一样对图片进行放大、缩小、旋转、复位、全屏等操作
 * [兼容] 兼容ie6+，但ie8及以下版本只能使用图片放大缩小功能。
 * [说明] 本控件在“jquery.magnify-v1.6.3”基础上进行错误修正和部分改写，更符合实际使用需求。原代码版权属于原作者所有，原作者主页：https://github.com/nzbin/magnify#readme
 * Modify: ChenMufeng
 * Date: 2021.09.15
 * Update: 2021.11.05
 */
  ...
  DEFAULTS = {
    ...
    icons: {
      ...
      fullscreen:
        ...
        // </svg>',
        '<svg viewBox="0 0 16 16" class="svg-inline-icon">\
          <path fill="currentColor"  d="M0 3.5A1.5 1.5 0 0 1 1.5 2h13A1.5 1.5 0 0 1 16 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 0 12.5v-9zM1.5 3a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-13z"/>\
          <path fill="currentColor" d="M2 4.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 0 1H3v2.5a.5.5 0 0 1-1 0v-3zm12 7a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1 0-1H13V8.5a.5.5 0 0 1 1 0v3z"/>\
        </svg>',
      actualSize:
      ...
      rotateRight:
        ...
        // '<svg viewBox="0 0 16 16" class="svg-inline-icon" fill="currentColor">\
...
var jqEl = null;
var jqElOrigin = null; // 原始控件里的jq对象

/**
 * Magnify Class
 * @param {string} method 图片点击Click事件触发方式. OUTER 外部, INNER 内部(默认) edit by chr 20211103
 * OUTER 外部，即在前端页面写Click事件，INNER 内部，即控件内部写Click事件
 */
var Magnify = function (el, options, method) {
  var self = this;
  ...
  this.isMinimized = true; // add by chr 20211103
...
Magnify.prototype = {
  init: function (el, opts) {
    ...
    // add by chr 20211103
    if(this.caller !== 'INNER'){
      imgSrc = opts.source.current;
    }

    if (currentGroupName !== undefined) {
      this.groupName = currentGroupName;
      this.getImageGroup(groupList, imgSrc);
    } else { // edit by chr 20211103
      // this.getImageGroup(jqEl.not('[data-group]'), imgSrc);
      var $jq = jqElOrigin == null ? jqEl : jqElOrigin;
      this.getImageGroup($jq.not('[data-group]'), imgSrc);
    }
  ...
  _createBtns: function (toolbar) {
    ...
    $.each(toolbar, function (index, item) {
      if ($.inArray(item, btns) >= 0) {
        ...
        btnsHTML +=
          '<button class="magnify-button magnify-button-custom magnify-button-' + item + '" title="' + (self.options.customButtons[item].title || '') + '">' +
  ...
  build: function () {
    ...
    // Add Magnify to DOM
    if($('.magnify-modal').length > 0) $('.magnify-modal').remove(); // add by chr 20211103
  ...
  getImageGroup: function (list, imgSrc) { // edit byc chr 20211103
    var self = this;
    self.groupData = [];
    if(this.caller == 'INNER'){     
      $(list).each(function (index, item) {
        ...
        if (imgSrc === _imgSrc){ // Get image index
          self.groupIndex = index;
        }
      })
    }
    else{
      var _imgSrc = this.options.source.current,
          imgLink = this.options.source.urls,
          imgText = typeof this.options.source.words == 'undefined' ? [ ] : this.options.source.words;
      self.groupIndex = 0;
      for(var i = 0; i < imgLink.length; i++){
        var index = i;
        var one = {
          src: imgLink[i],
          caption: imgText.length == 0 ? '' : imgText[i]
        }
        // console.log('one：', one)
        self.groupData.push(one);
        if (imgLink[i] === _imgSrc){
          self.groupIndex = index;
        }
      }
    }
  ...
  minimize: function () {
    ...
    if (this.isMinimized) {
      ...
      //   top: this.$magnify.offset().top
      // };
      // this.$magnify.addClass('magnify-minimize');
      // this.$magnify.css({
      //   width: '100%',
      //   height: '100%',
      //   left: 0,
      ...
      this.$magnify.removeClass('magnify-minimize');
      var initModalLeft = ($W.width() - this.options.modalWidth) / 2 + $D.scrollLeft();
      var initModalTop = ($W.height() - this.options.modalHeight) / 2 + $D.scrollTop();
      this.$magnify.css({
        width: this.modalData.width ? this.modalData.width : this.options.modalWidth,
  ...
  _addEvents: function () {
    ...
    this.$actualSize.off(CLICK_EVENT + EVENT_NS).on(CLICK_EVENT + EVENT_NS, function (e) {
      ...
      if (useIeTransforms) {
          self.$image.css('filter', 'progid:DXImageTransform.Microsoft.BasicImage(rotation=0)');
      }
    ...
    // add by chr 20211103
    this.$minimize.off(CLICK_EVENT + EVENT_NS).on(CLICK_EVENT + EVENT_NS, function () {
      self.minimize();
    });
...
$.fn.magnify = function (options, method) {
  var caller = typeof method == 'undefined' ? 'INNER' : method;
  jqEl = $(this);
  ...
  if (typeof options === 'string') {
    ...
    if(caller === 'INNER'){ // 事件在控件内部触发
      jqElOrigin = jqEl;
      jqEl.off(opts.initEvent + EVENT_NS).on(opts.initEvent + EVENT_NS, function (e) {
        e.preventDefault();
        e.stopPropagation(); // This will stop triggering data-api event
        $(this).data('magnify', new Magnify(this, options, caller));
      });
    }
    else{ // 事件在控件外部触发
      $(this).data('magnify', new Magnify(this, options, caller));
...
$.fn.neuiMagnify = function(options){
    ...
    var tips = '';
    if(method == 'OUTER'){
      if($.isEmptyObject(options.source)) tips = 'SOURCE参数为空，请检查！';
      else if(typeof options.source.urls == 'undefined' || typeof options.source.current == 'undefined') tips = '请传递参数URLS和CURRENT！';
      else if(options.source.current.toString().replace(/([ ]+)/g, '') === '') tips = '参数CURRENT为空，请查询！';
      else if(!$.isArray(options.source.urls)) tips = '参数URLS不是数组，请检查！';
      else if(options.source.urls.length == 0) tips = '参数URLS数组为空，请检查！';
    }
    if(tips != ''){
      alert(tips);
      return;
    ...
    $(this).magnify(options, method);
};


/**
 * MAGNIFY DATA-API
 * 自动加载控件，只要页面上 data-magnify 属性 的标签 edit by chr 20211103
 */
